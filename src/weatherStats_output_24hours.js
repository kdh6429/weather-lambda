...
module.exports.output = (event, context, callback) => {
        ...
        try {
            delay(index * 3000).then(() => {
                ...
                //commonUtil.getData("weather-output", docId).then(localDoc => {
                    get24hourlyDoc(event, map_info, localDoc).then(doc => {
                        doc["id"] = docId;
    ...
    Promise.allSettled(statsDocsPromises).then( datas => {
        ...
        commonUtil.sendtoSlack("Update weather-output 24hourly : " + docs.length).then( () => {
            if( docs.length === 0) {
                callback(null, response("no data to update"));
                return ;
            }
            commonUtil.getFromS3("lastest.json").then(totalDoc => {
                get24hourlyTotalDoc(event, docs, totalDoc).then(doc => {
                    // total doc
                    doc["id"] = "lastest";
                    doc["update"] = new Date().getTime();
                    ...
                    //importData("weather-output", [doc]).then( count => {
                        commonUtil.sendtoSlack("Update weather-output 24hourly : lastest").then( () => {
...
const get24hourlyTotalDoc = async(event, docs, totalDoc) => new Promise((resolve, reject) => {
    const {yyyymmdd, hour} = commonUtil.getDateTime(event);
    const timeKeys = ['d3','d4','d5','d6','d7','d8','d9','d10'];
...
const get24hourlyDoc = async(event, map_info,  localDoc) => new Promise((resolve, reject) => {
    const {yyyymmdd, hour} = commonUtil.getDateTime(event);
    const hourlyStatsKey = "24hourly-" + yyyymmdd + "." + hour + "00." + map_info.x + "." + map_info.y;
    ...
    commonUtil.getData("weather-hourly-stats", hourlyStatsKey).then(rtnData => {
        ...
        for(const index in timeKeys) {
            ...
            if('wf_e' in hourlyStats[timeKey] && 'tpy_a' in hourlyStats[timeKey] && 
                (hourlyStats[timeKey]['wf_e'].includes('비') || 
                hourlyStats[timeKey]['wf_e'].includes('눈') || 
                hourlyStats[timeKey]['wf_e'].includes('소나기') || 
                hourlyStats[timeKey]['tpy_a'].some(c => c && c !== '0'))) {
                const tmpObj = {
                    't' : commonUtil.convertYMDHtoDate(yyyymmdd, hour).getTime(),
                    'e' : hourlyStats[timeKey]['wf_e'],
                    'a' : hourlyStats[timeKey]['tpy_a'],
                    'f': ( ()=> {
                        return (hourlyStats[timeKey]['wf_e'].includes('비') || 
                        hourlyStats[timeKey]['wf_e'].includes('눈') || 
                        hourlyStats[timeKey]['wf_e'].includes('소나기')) && hourlyStats[timeKey]['tpy_a'].some(c => c && c !== '0')
                    })()
                };
                ...
                // 강수 확률 
                if ( !( 'pop_' + hourlyStats[timeKey]['rnSt_e'] in localDoc[timeKey])) {
                    localDoc[timeKey]['pop_' + hourlyStats[timeKey]['rnSt_e']] =  { 'raw':[], 'history' : [], 'rate' : 0 };
                }
                localDoc[timeKey]['pop_' + hourlyStats[timeKey]['rnSt_e']]["raw"] = commonUtil.addItemByTime(localDoc[timeKey]['pop_' + hourlyStats[timeKey]['rnSt_e']]["raw"], {
                    't' : commonUtil.convertYMDHtoDate(yyyymmdd, hour).getTime(),
                    'f' : hourlyStats[timeKey]['tpy_a'].some(c => c && c !== '0')
                }, LOCAL_HISTORY_LENGTH);
                localDoc[timeKey]['pop_' + hourlyStats[timeKey]['rnSt_e']]["rate"] = (() => {
                    return localDoc[timeKey]['pop_' + hourlyStats[timeKey]['rnSt_e']]["raw"].filter( obj=> obj.f).length / localDoc[timeKey]['pop_' + hourlyStats[timeKey]['rnSt_e']]["raw"].length;
                })();
                
                localDoc[timeKey]['pop_' + hourlyStats[timeKey]['rnSt_e']]["history"] = commonUtil.addItemByTime(localDoc[timeKey]['pop_' + hourlyStats[timeKey]['rnSt_e']]["history"], {
                    't' : commonUtil.convertYMDHtoDate(yyyymmdd, hour).getTime(),
                    'r' : localDoc[timeKey]['pop_' + hourlyStats[timeKey]['rnSt_e']]["rate"]
                }, LOCAL_HISTORY_LENGTH);
            }
